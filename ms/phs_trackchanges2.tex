...
Such trait data have been shown to improve predictions of species vulnerability to drought \citep{choat2012,mackay2015,powell2018,giardina2018,anderegg2018} \add{[citation added]}.
...
\item \change{Modeling}{Using a Darcy's Law approximation to model} plant hydrodynamics allows representation of hydraulic redistribution \citep{lee2005}, as \change{the flow respects Darcy's law and thus is}{water fluxes are} always directed down gradients of water potentials. 
\item Root water potential can be used to predict gradient-based root water uptake \change{based on}{approximated by} Darcy's law, replacing the previous empirical transpiration partitioning heuristic. This provides the means to vary, for example, the mean depth of extraction with changing soil water conditions.
\item Representation of a range of water use strategies (e.g. isohydricity and anisohydricity), improving the connection between \change{plant carbon allocation}{leaf gas exchange} and water availability.
...
\label{sect:gs}
    CLM5 implements the Medlyn stomatal conductance model \citep{franks2018,medlyn2011}, which reconciles the empirical approach to modeling stomatal conductance with a carbon-water optimization framework \citep{cowan1977}.
    \add{The} \cite{cowan1977} \add{optimization requires plants to maximize photosynthesis relative to transpiration costs, which the Medlyn model captures in an empirically tractable form (Equation 1).}
    ...
    \add{The PHS implementation follows this approach, using the Medlyn model to calculate stomatal conductance absent water stress, which is attenuated as leaf water potential declines (see Section} \ref{sect:wsf}).
    \add{More recent stomatal conductance models eschew the} \cite{cowan1977} \add{optimization in favor of a framework to maximize carbon assimilation relative to hydraulic costs} \citep{wolf2016,sperry2017,anderegg2018b} \add{, which directly incorporate leaf water potential and hydraulic safety margin into the stomatal conductance formulation.
...
\label{sect:smsrwu}
    ...
    Transpiration is partitioned among the soil layers based on the product of the root fraction and the wilting factor, which is then normalized by $f_{w,\text{SMS}}$ to satisfy Equation 5. \add{Because the relative root fractions are used to partition transpiration, root water uptake is not connected to absolute root biomass. For example, if root biomass doubles in every soil layer, the relative root fractions do not change, with no impact on modeled soil water availability.}
    \begin{linenomath*}
    \begin{equation}
    \label{bt:4}
...
\psi_{\text{leaf}} = \psi_{\text{soil}} + \Delta\psi
    ...
    As a result, low soil water \remove{(bottom-up stress)} induces stress due to limited water supply, but in addition, high atmospheric VPD can induce stress with the associated increases in the gradient in water potential across the plant xylem \remove{(top-down stress)}. 
    ...
    PHS implements an alternative to the SMS heuristic approach for root water uptake, using a mechanistic representation utilizing Darcy's Law \add{for flow through porous media to approximate the vegetation water fluxes}.
    ...
    accounting for both the path across the soil matrix and through the xylem conduits. \add{Furthermore, in lieu of using relative root fraction (as in SMS), $k_{sr}$ depends on the root area index, an absolute measure of root abundance (details in Appendix B).}
...
\label{sect:vwp}
  The PHS model within CLM5 uses Darcy's law to \change{model}{approximate} the flow of water through the SPAC, which can be represented with an electrical circuit analogy (Figure \ref{circuit}).
...
%Water Supply
    ...
    whereby its inclusion may be warranted in future model versions.
     ...
     \add{This bias could underestimate hydraulic limits on gas exchange and/or affect parameter estimation (e.g. requiring lower $k_{\text{max}}$).}
     ...
     \add{PHS was explicitly designed as a simplified model that can be refined in future versions.}
...
\label{sect:ttfe}
PHS performs better than SMS at reproducing transpiration observations under TFE (Figure \ref{fig:t}b,d), \add{featuring a higher} R$^2$ \add{(0.45 vs. 0.3)} and \add{lower} RMSE \add{(0.74 vs. 1.03 mm/d)}.
\add{However, both implementations show degraded results under TFE as compared to ambient rainfall conditions.}
...
\add{The sensitivity of leaf gas exchange to transpiration demand is subject to the representation of hydraulic conductance in PHS, which requires caveats related to parametric uncertainty and hydraulic simplifications} (see Section \ref{sect:supply}).
...
\subsection{Can modeling vegetation water potential improve the CLM?}
    ...
    \add{Darcy's Law, which is already used to model soil water movement in the CLM, offers a useful approximation for vegetation water fluxes} \citep{sperry1998}.
...
The PHS stress formulation will impart an increased sensitivity of GPP to rising VPD under climate change, which could lead to a diminished terrestrial carbon sink relative to SMS projections.
    ...
    \add{The PHS formulation represents an incremental approach to coupling stomatal conductance to leaf water potential, inheriting the established CLM stomatal conductance model} \citep{franks2018}, \add{while adapting the water stress factor to depend on leaf water potential. This could be refined in future versions of PHS, incorporating recent work that directly incorporates leaf water potential within the stomatal optimization} \citep{wolf2016,sperry2017,anderegg2018b}.
...
Dynamism in the gradient of water potential from soil-to-leaf, according to \change{Darcy's Law}{the Darcy's Law approximation of vegetation water fluxes}, drives RWU.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \change{Several other approaches have been described in the literature, and future research is needed to elucidate water stress dynamics and mechanisms}{However, significant uncertainty exists in coupling the PHS water stress factor to the Medlyn stomatal conductance model, which could be resolved in future versions by recent work that directly incorporates hydraulic limitations within the stomatal optimization} \citep{wolf2016,sperry2017,anderegg2018b}.
    \end{itemize}
...
\section{Figures}
  ...
  \end{figure}
    ...
    \begin{figure}[h]
     \centering
     \includegraphics[width=30pc]{sm2.pdf}
     \caption{Volumetric soil water content (-) over time under ambient and TFE conditions at a depth of 50cm.
     (a/b) SMS
     (c/d) PHS.
     Arrows indicate start of TFE. Supplementary Figure S11 plots 7 other soil depths. In the SMS configuration, soil moisture can tend to `stick' at water content of 0.1 during the dry season, which corresponds to $\psi_{\text{soil}}$=-2.5MPa and is the value of the SMS soil wilting parameter, $\psi_c$. }
     \label{fig:sm2}
  \end{figure}
    ...
    \begin{figure}[h]
     \centering
     \includegraphics[width=30pc]{vwp.pdf}
...
The new root water uptake parameterization allows PHS to maintain more transpiration under TFE by utilizing more deep soil water.}
  ...
  \end{figure}
              ...
              \begin{figure}[h]
     \centering
     \includegraphics[width=30pc]{t_err.pdf}
...
The numerics have proven to be stable in practice, but future versions may aim to better integrate PHS within the stomatal conductance scheme to improve the coherence of Equations \ref{eq:d1} and \ref{supp:fw}.
    ...
    \end{equation}
         ...
         \begin{figure}[h]
     \centering
     \includegraphics[width=30pc]{schem3.pdf}
